...
var otherMac={
		...
		initUI:function (){
			$('#otherMacListTab').datagrid({
				 ...
				 columns:[[
					   	        ...
					   	        {field:'time',title:'出现时间' ,formatter:timestampToyyyyMMddHHmmssFormatter },
					   	        {field:'ip',title:'ip'   }
				 	   	    ]]
			});
		...
		uiShowImportConfirm:function(){
			...
			$.messager.confirm('Confirm','确定要导入吗<br>'+list,
				function(r){
			    if (r){
			    	//选择型号
			    	//showComboxBox
		...
		fnAddMacs:function(){
			var deviceCode=$('#otherMacChooseModelNoCombobox').combobox('getValue');
			if(deviceCode==null){
			 ...
			 $.ajax({
					...
					async:true,
					data:{"macs" : otherMac.macs,"deviceCode": deviceCode},
					error:function(request){
		...
		uiShowDelMacConfirm: function(){
			...
			$.messager.confirm('Confirm','确定要删除吗吗<br>'+list,
				function(r){
			    if (r){
