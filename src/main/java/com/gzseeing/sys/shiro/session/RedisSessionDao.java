...
public class RedisSessionDao extends EnterpriseCacheSessionDAO {
	...
	protected Serializable doCreate(Session session) {
		   Serializable sessionId = super.doCreate(session);
	        ...
	        objectRedisDao.saveValueInMap(Dictionary.APP_USER+":"+sessionId, ShiroConfig. MAP_SHIRO_KEY,  session , 1L*SysConfig.CONFIG_APP_SESSION_TIMEOUT_INT);
	        return sessionId;
	}
	...
	protected Session doReadSession(Serializable sessionId) {
        RedisDao<String, Object> objectRedisDao = RedisUtils.getObjectRedisDao();
        Session session = objectRedisDao.getByMapKey(Dictionary.APP_USER+":"+sessionId, ShiroConfig. MAP_SHIRO_KEY, Session.class);
        return session;
	}
	...
	protected void doUpdate(Session session) {
        ...
        RedisDao<String, Object> objectRedisDao = RedisUtils.getObjectRedisDao();
        objectRedisDao.saveValueInMap(Dictionary.APP_USER+":"+session.getId(), ShiroConfig. MAP_SHIRO_KEY, session, 1L*SysConfig.CONFIG_APP_SESSION_TIMEOUT_INT);
